 ...
 *
 * When starting a thread, you have to keep on checking if the thread is still alive. This is to prevent zombie processes.
 */
class Thread {
     ...
     * @return null
     * @throws zibo\ZiboException when process control is not supported  by your PHP configuration
     */
    public function __construct($callback) {
        if (!function_exists('pcntl_fork')) {
            throw new ZiboException('Could not create a Thread instance. Your PHP installation does not support process control, please install the pcntl plugin.');
        }
     ...
     * @return null
     * @throws zibo\ZiboException when an invalid signal is provided
     */
    public function interrupt($signal = SIGKILL, $wait = false) {
        ...
        if ($signal != SIGKILL && $signal != SIGTERM) {
            throw new ZiboException('Could not interrupt the process: invalid signal provided, try SIGKILL or SIGTERM');
        }

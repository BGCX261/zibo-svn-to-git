...
class HeaderContainer implements Iterator, Countable {
    ...
    /**
     * Sets the headers from the $_SERVER variable
     * @return null
     */
    public function setHeadersFromServerRequest() {
        foreach ($_SERVER as $key => $value) {
            if (substr($key, 0, 5) != 'HTTP_' || !$value) {
                continue;
            }

            $this->setHeader(substr($key, 5), $value);
        }
    }
     ...
     * @param string $value Value of the directive, true as flag, a value otherwise
     * @return null
     * @throws zibo\ZiboException when the directive is empty or not a string
     */
    public function addCacheControlDirective($directive, $value = true) {
        if (!String::isString($directive, String::NOT_EMPTY)) {
            throw new ZiboException('Provided directive is empty or not a string');
        }

        if ($value !== true && !String::isString($value, String::NOT_EMPTY)) {
            throw new ZiboException('Provided value is empty or not a string');
        }
     ...
     * @param string $directive Name of the directive
     * @return boolean|string|null The value of the directive if found, null otherwise
     * @throws zibo\ZiboException when the directive is empty or not a string
     */

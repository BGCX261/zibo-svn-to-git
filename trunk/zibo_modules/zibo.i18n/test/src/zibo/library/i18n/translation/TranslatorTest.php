...
class TranslatorTest extends BaseTestCase {
    ...
    public function setUp() {
        $this->englishLocale = new Locale('en', 'English', '$n != 1');
        $this->dutchLocale = new Locale('nl', 'Nederlands', '$n != 1');
        $this->frenchLocale = new Locale('fr', 'français', '$n > 1');
    ...
    public function testConstruct() {
        ...
        $this->assertEquals($this->englishLocale->getCode(), Reflection::getProperty($translator, 'locale'));
        $this->assertEquals($this->englishLocale->getPluralScript(), Reflection::getProperty($translator, 'pluralScript'));
    }
    ...
    public function providerTranslatePluralUsesPluralScriptFromLocale() {
        $englishLocale = new Locale('en', 'English', '$n != 1');
        $dutchLocale = new Locale('nl', 'Nederlands', '$n != 1');
        $frenchLocale = new Locale('fr', 'français', '$n > 1');
        $russianLocale = new Locale('ru', 'русский', '($n % 10 == 1 && $n % 100 != 11) ? 0 : ($n % 10 >=2 && $n % 10 <=4 && ($n % 100 < 10 || $n % 100 >= 20) ? 1 : 2)');

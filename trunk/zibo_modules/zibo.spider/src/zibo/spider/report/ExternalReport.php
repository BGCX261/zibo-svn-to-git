...
/**
 * Spider report of all the nodes which are external references
 */
class ExternalReport implements SpiderReport {

    /**
     * Translation key for the title of this report
     * @var string
     */
    const TRANSLATION_TITLE = 'spider.title.report.external';
    ...
    public function setWeb(Web $web) {
        ...
        foreach ($nodes as $node) {
            if (!$node->hasType(WebNode::TYPE_EXTERNAL) || $node->hasType(WebNode::TYPE_IGNORED)) {
                continue;
            }

...
class NestedSymbol extends AbstractSymbol {
    ...
    /**
     * Optional tokenizer to tokenize the value between the open and close symbol
     * @var zibo\library\tokenizer\Tokenizer
     */
    protected $tokenizer;
     ...
     * @param zibo\library\tokenizer\Tokenizer $tokenizer When provided, the value between the open and close symbol
     *                                                    will be tokenized using this tokenizer
     * @param boolean $willIncludeSymbols True to include the open and close symbol in the tokenize result, false otherwise
     * @return null
     */
     ...
     * @param integer $initialOpenPosition The position of the open symbol for which to find the close symbol
     * @return integer The position of the close symbol
     * @throws zibo\ZiboException when the symbol is opened but not closed
     */
    protected function getClosePosition($string, $initialOpenPosition) {
        ...
        if ($closePosition === false) {
            throw new ZiboException($this->symbolOpen . ' opened (at ' . $initialOpenPosition . ') but not closed for ' . $string);
        }
     ...
     * @param string $symbol
     * @return null
     * @throws zibo\ZiboException when the provided symbol is empty or not a string
     */
    private function setOpenSymbol($symbol) {
        if (String::isEmpty($symbol)) {
            throw new ZiboException('Provided open symbol is empty');
        }
    ...
    private function setCloseSymbol($symbol) {
        if (String::isEmpty($symbol)) {
            throw new ZiboException('Provided close symbol is empty');
        }

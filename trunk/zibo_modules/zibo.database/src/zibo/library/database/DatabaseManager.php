...
class DatabaseManager {
    ...
    public function setDefaultConnectionName($defaultConnectionName) {
        if (!String::isString($defaultConnectionName, String::NOT_EMPTY)) {
    ...
    public function registerDriver($protocol, $className) {
        if (!String::isString($protocol, String::NOT_EMPTY)) {
    ...
    private function loadDriversFromConfig(Zibo $zibo) {
        $zibo->triggerEvent(Zibo::EVENT_LOG, 'Loading database drivers', '', 0, self::LOG_NAME);

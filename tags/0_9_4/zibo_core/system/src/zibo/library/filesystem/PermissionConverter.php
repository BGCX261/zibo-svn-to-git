...
class PermissionConverter {
     ...
     * @throws zibo\ZiboException when the provided permissions is not a valid numeric permissions value
     */
     ...
     * @param string $rwxPermissions Permissions in the rwx format (eg. rwxr-xr-x)
     * @return int Numeric permissions (eg. 755)
     * @throws zibo\ZiboException when the provided permissions is not a valid rwx permissions value
     */
    ...
    private function checkNumericPermissions($numericPermissions) {
        if (!preg_match(self::REGEX_NUMERIC, $numericPermissions)) {
            throw new ZiboException('Provided permissions is not a valid numeric permissions value: value not between 000 and 777');
        }
    }
    ...
    private function checkRwxPermissions($rwxPermissions) {
        if (!preg_match(self::REGEX_RWX, $rwxPermissions)) {
            throw new ZiboException('Provided permissions is not a valid rwx permissions value: only r, w, x and - are allowed and the length of the permissions should be 9 characters long.');
        }
    }

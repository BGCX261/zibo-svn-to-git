...
/**
 * Definition and helper of a generic automated task
 */
class GenericCronJob implements CronJob {
	...
	/**
	 * Time this jobs has run the last time
	 * @var int
	 */
	...
	public function __toString() {
		return $this->callback->__toString() . ' (' . $this->intervalDefinition . ')';
	}
    ...
    public function run() {
    	$args = func_get_args();
    	try {
    	   $this->callback->invokeWithArrayArguments($args);
	...
	protected function parseRunIntervalValue($value, $min, $max) {
		if (String::isEmpty($value)) {
			throw new ZiboException('Empty value provided');
		}
		...
		foreach ($explodedValue as $value) {
		    ...
		    if ($posSeparatorIncrement !== false) {
		        ...
		        if (!is_numeric($incrementValue)) {
                    throw new ZiboException('Invalid increment value provided: ' . $incrementValue);
		        }
			...
			if ($posSeparatorRange !== false && $posSeparatorRange != 0) {
                ...
                if ($range[0] > $range[1]) {
                	throw new ZiboException($range[0] . ' is greater then ' . $range[1] . ' in ' . $value);
                }
	...
	/**
     * Validates a interval value. A numeric value between min and max is required (min and max included)
     * @param mixed $value
     * @param int $min
     * @param int $max
     * @return null
     * @throws zibo\ZiboException when the value is not valid
	 */
	protected function validateRunIntervalValue($value, $min, $max) {
		if (!is_numeric($value)) {
            throw new ZiboException('Invalid value provided: ' . $value);
		}

		if ($value < $min || $max < $value) {
			throw new ZiboException('Invalid value provided: ' . $value . ' should be between ' . $min . ' and ' . $max);
		}
	}
